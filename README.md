...
- **~12ms** cold startup — 40x faster than Node.js agents
- **~9ms** config parsing — instant-on from any environment
...
|---|---|---|
| `single` | ✅ **Ready** | Standalone autonomous agent |
| `swarm` | 🔄 In progress | Multiple coordinated agents |
| `supervisor` | 🔄 In progress | Orchestrator for sub-agents |
...
| Fail-closed by default | ✅ | ❌ | ❌ | Partial |
| No credentials in config | ✅ | ✅ | ❌ | ❌ |
| Read-only container | ✅ | ❌ | ❌ | ❌ |
| Non-root container | ✅ | ❌ | ❌ | ❌ |
| Audit logging | ✅ | ✅ | ❌ | ✅ |
| Prompt injection defense | 🔄 In progress | ❌ (bypassed) | ❌ | Partial |
...
│  │  TLS · Audit · Rate Limit · Metrics · Tracing  │   │
│  └─────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────┘
         │                          │
         ▼                          ▼
...
│   Coordination  │      │  Binary · Docker · K8s │
└─────────────────┘      └──────────────────────┘
```
...
| **Startup time** | **~12ms** | ~500ms+ | ~2s+ | ~1s+ |
| **Memory per agent** | **~5MB** | ~50MB+ | ~100MB+ | ~80MB+ |
| **CVEs (2026)** | **0** | 15+ | N/A | N/A |
| **Multi-provider** | ✅ 4 providers | Plugin-based | OpenAI-centric | ✅ |
| **Swarm mode** | 🔄 Native Rust | Via plugins | Via Python | Via gateway |
| **Verification tests** | **38** | Limited | Community | Internal |
