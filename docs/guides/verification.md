...
In addition, **507 Rust unit tests** run via `cargo test` covering all 21 source modules (agent, config, error, llm, tools, mcp, server, telemetry, policy, audit, sandbox, ravenfabric, background, scheduler, eval, swarm, heartbeat, patterns, persistence, plugins, load, lib).
...
cargo test                             # Run 507 Rust unit tests
